...
namespace wayeal.validate
{
    ...
    {
        public ValidateBase(object expression,string errorText)
        {
            Expression = expression;
            ErrorText = errorText;
        }
        public ValidateBase(object expression, object expression2, string errorText)
        {
            Expression = expression;
        ...
        /// 验证表达式2
        /// </summary>
        public object Expression2 { get; set; }
        /// <summary>
        /// 验证失败后的错误信息
        /// </summary>
        ...
        /// <returns></returns>
        public virtual ValidateResult Validate(object value)
        {
            ValidateResult r = new ValidateResult();
            r.Valid = true;
            if (Expression == null) return r;
            
            Regex reg = new Regex(Expression.ToString());
            r.Valid= reg.Match(value.ToString()).Success;
            r.ErrorTextKey = ErrorText;
            return r;
        }
        /// <summary>
        /// 验证结果
        ...
        /// <param name="value">待验证值</param>
        /// <returns></returns>
        public virtual ValidateResult Validate(string pattern, object value)
        {
            ValidateResult r = new ValidateResult();
            r.Valid = true;
            if (pattern == null) return r;
